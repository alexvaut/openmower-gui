	...
	group.GET("/subscribe/:topic", func(c *gin.Context) {
		...
		*/
		...
		case "stallBreakerParams":
			def, err = subscribe(provider, c, conn, "/ll/stall_breaker/parameter_updates", -1)
		case "gps":
...
func ServiceRoute(group *gin.RouterGroup, provider types.IRosProvider) {
	...
	group.POST("/call/:command", func(c *gin.Context) {
		...
		case "stall_breaker":
			...
			err = provider.CallService(c.Request.Context(), "/ll/stall_breaker/set_parameters", &dynamic_reconfigure.Reconfigure{}, &CallReq, &dynamic_reconfigure.ReconfigureRes{})
		case "mow_enabled":
